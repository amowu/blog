...
describe('訪問管理後台特定頁面', () => {
  before(() => {
    // 這是自訂的 Cypress Custom Command，用於快速登入某個帳號，這裡就不細述實作
  ...
  describe('從側欄連結先訪問「課程管理」再訪問「成效追蹤」', () => {
    it('點擊左側的「課程管理」選項，右側標題應該顯示「課程管理」', () => {
      // 可以給要操作的 element 加上 data-test-id attribute，這樣 selector 會比較好寫
